...
// ── OAUTH CALLBACK ────────────────────────────────
// Deriv redirects here after successful login
app.get('/callback',(req,res)=>{
  const token1 = req.query.token1 || '';
  const botId  = req.query.state  || '1';
  if(!token1){
    res.send('<script>window.location="/?error=no_token"</script>');
    return;
  }
  // Auto-apply token to bot and redirect to dashboard
...
<div class="spin">⚡</div>
<p>AUTHENTICATED</p>
<p style="color:#ffd60a;font-size:11px">Connecting Bot ${botId}...</p>
<script>
// Store token and bot ID then go to dashboard
sessionStorage.setItem('oauth_token_${botId}', '${token1}');
