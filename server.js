...
// Works with both legacy and new PAT tokens
// ═══════════════════════════════════════════════
'use strict';
...
const APP_ID = '33jX6j2IcffFY9Etm3pHU';
...
function createBot(id) {
  ...
  return {
    ...
    // Structures — multiple simultaneously
    activeStructures:[],
    // DO NOT TRADE zones — no trading at all inside
    noTradezones:[],
    // HTF zones — uptrend detection only, bot pauses if uptrend forms
    htfZones:[],
    htfZonePaused:false,
    // Ignored specific levels
...
function findLevels(b) {
  ...
  // Telegram: level alert when 2+ TFs confirm downtrend
  const dc=Object.values(b.trendStatus).filter(t=>t==='down').length;
  if(dc>=2){
...
// ONE projected level per structure
function computeProjected(b,struct,tradedLevels) {
  ...
  let safety=0;
  while(safety<20){
    // Skip if: already traded, in NO TRADE zone, or specifically ignored
...
// DO NOT TRADE zone check — completely blocks trading
...
// HTF zone check — price proximity only
...
// When price is in HTF zone, watch for uptrend structure on M1
// Higher low + break above previous swing high = pause bot
function checkHTFZone(b) {
  ...
  if(!inZone){
    // Price left zone downward = resume
    if(b.htfZonePaused){
      const stillAboveAny=b.htfZones.some(z=>b.currentPrice>=Math.min(z.a,z.b)*0.998);
      if(!stillAboveAny){
        log(b,'✅ Price broke out of HTF zone downward — resuming');
  ...
  // Check if all M1/M5/M15 turned uptrend — safe to resume
  ...
  // M1 must be downtrend to watch for uptrend structure in zone
  if(b.trendStatus['M1']!=='down') return;

  // Find swing lows and highs in recent M1 candles
  ...
  // Uptrend structure = higher low formed + price broke above previous swing high
...
function startLossCountdown(b,secs) {
  ...
  log(b,`⏸ Cooldown: ${secs===1800?'30MIN':secs===3600?'1HR':'4HR'}`);
...
function startTimeOff(b,secs) {
  ...
  log(b,`⏰ Time off: ${secs===1200?'20MIN':secs===1800?'30MIN':'1HR'}`);
...
// ── DERIV CONNECTION ──────────────────────────────
// Works with both legacy tokens and new PAT tokens
// Deriv support confirmed: same authorize flow for server-side bots
...
dashWss.on('connection',(ws)=>{
  ...
  ws.on('message',(raw)=>{
    ...
    // HTF zones — uptrend detection only
    if(msg.type==='add_htf'){
...
// Opens Deriv login page
app.get('/login',(req,res)=>{
  const botId = req.query.bot || '1';
  const redirectUri = encodeURIComponent('https://betterwork-iyd6.onrender.com/callback');
  const url = `https://oauth.deriv.com/oauth2/authorize?app_id=${APP_ID}&response_type=token&redirect_uri=${redirectUri}&state=${botId}`;
  res.redirect(url);
});

// ── OAUTH CALLBACK ────────────────────────────────
// Deriv redirects here — token is in URL params or fragment
// We handle both cases client-side
app.get('/callback',(req,res)=>{
  res.send(`<!DOCTYPE html>
<html>
<head>
<title>EL ROI — Connecting...</title>
<style>
body{background:#03060f;color:#00ff9d;font-family:'Share Tech Mono',monospace;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;flex-direction:column;gap:16px;text-align:center;}
.spin{font-size:48px;animation:sp 0.8s linear infinite;}
@keyframes sp{to{transform:rotate(360deg)}}
p{font-size:13px;letter-spacing:2px;color:#8ab0c8;}
.err{color:#ff2d55;font-size:11px;}
</style>
</head>
<body>
<div class="spin">⚡</div>
...
<script>
(function(){
  // Deriv sends token as query params: ?token1=xxx&state=botId
  // OR as hash fragment: #token1=xxx
  var search = window.location.search;
  var hash   = window.location.hash;

  // Try query params first
  var params = new URLSearchParams(search);
  var token  = params.get('token1');
  var botId  = params.get('state') || '1';

  // Fallback: try hash fragment
  if(!token && hash){
    var hParams = new URLSearchParams(hash.replace('#',''));
    token  = hParams.get('token1');
    botId  = hParams.get('state') || botId;
  }

  if(!token){
    document.getElementById('msg').textContent = 'NO TOKEN RECEIVED';
    document.getElementById('err').textContent = 'URL: ' + window.location.href;
    setTimeout(()=>{ window.location = '/'; }, 3000);
    return;
  }

  document.getElementById('msg').textContent = 'BOT ' + botId + ' AUTHENTICATED ✅';

  // Save to localStorage so it persists
  try {
    var saved = JSON.parse(localStorage.getItem('elroi_cfg') || '{}');
    if(!saved[botId]) saved[botId] = {};
    saved[botId].apiToken = token;
    localStorage.setItem('elroi_cfg', JSON.stringify(saved));
    localStorage.setItem('elroi_oauth_bot', botId);
  } catch(e){}

  // Also save to sessionStorage for immediate pickup
  sessionStorage.setItem('oauth_token_' + botId, token);
  sessionStorage.setItem('oauth_bot', botId);
