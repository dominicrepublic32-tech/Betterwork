...
function checkHTFZoneUptrend(b) {
  ...
  if(!nearZone){
    if(b.htfZonePaused){
      const stillInAny=b.htfZones.some(z=>{
  ...
  if(b.htfZonePaused&&upCount>=3){
    log(b,'✅ All TFs uptrend — HTF zone resolved, waiting for next downtrend');
    b.htfZonePaused=false;
    setStatus(b,'running','RUNNING');
...
dashWss.on('connection',(ws)=>{
  ...
  ws.on('message',(raw)=>{
    ...
    if(msg.type==='force_trade'){
      if(!b.botActive){ broadcast({type:'error',id:b.id,msg:'Bot not running'}); return; }
      if(b.inTrade){ broadcast({type:'error',id:b.id,msg:'Trade already open'}); return; }
      log(b,'⚡ FORCE TRADE triggered manually');
      placeTrade(b);
    }
