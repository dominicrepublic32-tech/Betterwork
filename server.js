...
const APP_ID_LIVE   = '33kbRhT3vWWKhrOsdu0vN'; // default — overridable in settings
...
function createBot(id) {
  ...
  return {
    ...
    cfg: {
      ...
      scanTF:'M1+M5',
      scanTFs:['M1','M5'], // selectable timeframes for structure scanning minTFConfirm:2, smallTol:10, bigTol:15,
      ...
      liveAppId:'33kbRhT3vWWKhrOsdu0vN',
    ...
    trendStatus:{ M1:null,M5:null,M15:null,M30:null },
...
function isPriceInHTFZone(b,price) {
  const allZones=[...b.htfZones,...(b.autoHtfZones||[])];
...
// ── AUTO HTF ZONE DETECTION ───────────────────────
// Finds swing low structures on H1, H4, D1
// 3 types: ascending, descending, equal/almost equal
function findAutoHTFZones(b) {
  ...
  ['H1','H4','D1'].forEach(tf=>{
    ...
    // Find swing lows (lower than 3 candles each side)
    const LR=3, swingLows=[];
    for(let i=LR;i<data.length-LR;i++){
      let isLow=true;
      for(let j=i-LR;j<=i+LR;j++){
        if(j!==i&&data[j].low<=data[i].low){isLow=false;break;}
      }
      if(isLow) swingLows.push({
        low:data[i].low, high:data[i].high,
        range:data[i].high-data[i].low, idx:i
      });
    }
    if(!swingLows.length) return;

    // Use last 3 swing lows only
    const recent=swingLows.slice(-3);

    // ── Equal/Almost Equal cluster ──
    if(recent.length>=2){
      const lows=recent.map(s=>s.low);
      const minLow=Math.min(...lows), maxLow=Math.max(...lows);
      if((maxLow-minLow)/minLow<0.005){ // within 0.5%
        const clusterHigh=Math.max(...recent.map(s=>s.high));
        const totalRange=clusterHigh-minLow||1;
        allZones.push({
          a:minLow+(closePct*totalRange),
          b:minLow-(passPct*totalRange),
          type:'equal', tf, auto:true
        });
        return;
      }
    }

    // ── Ascending or Descending (1,2 and NEXT) ──
    if(recent.length>=2){
      const sl1=recent[recent.length-2];
      const sl2=recent[recent.length-1];
      const baseDiff=Math.abs(sl1.low-sl2.low);
      if(baseDiff>0){
        const ascending=sl2.low>sl1.low;
        const nextLow=ascending ? sl2.low+baseDiff : sl2.low-baseDiff;
        allZones.push({
          a:nextLow+(closePct*baseDiff),
          b:nextLow-(passPct*baseDiff),
          type:ascending?'ascending':'descending',
          tf, auto:true, nextLow
        });
      }
    }

    // ── Single swing low (Pattern 1 and 2) ──
    const sl1=recent[recent.length-1];
    const range=sl1.range||1;
    const zoneA=sl1.low+(closePct*range);
    const zoneB=sl1.low-(passPct*range);
    if(b.currentPrice>zoneA){
      allZones.push({a:zoneA, b:zoneB, type:'single', tf, auto:true});
    }
  });

  // Keep only zones below current price, max 5 closest
  b.autoHtfZones=allZones
    .filter(z=>b.currentPrice>Math.min(z.a,z.b))
    .sort((x,y)=>Math.abs(x.a-b.currentPrice)-Math.abs(y.a-b.currentPrice))
    .slice(0,5);

  broadcast({type:'auto_htf_zones',id:b.id,zones:b.autoHtfZones});
}

// ── FIND LEVELS — EVERY SECOND ────────────────────
function findLevels(b) {
  // Use scanTFs array if set, otherwise fall back to scanTF string
  const tfs = (b.cfg.scanTFs && b.cfg.scanTFs.length)
    ? b.cfg.scanTFs
    : b.cfg.scanTF==='M1'?['M1']:b.cfg.scanTF==='M5'?['M5']:['M1','M5'];
  const newStructures=[];
...
function checkHTFZoneUptrend(b) {
  const allZones=[...b.htfZones,...(b.autoHtfZones||[])];
  if(!allZones.length||!b.currentPrice) return;

  const nearZone=isPriceInHTFZone(b,b.currentPrice);

  if(!nearZone){
    if(b.htfZonePaused){
      // Resume when price breaks below Zone B
      // Resume when price breaks BELOW zone B (the lower boundary)
      const stillInAny=allZones.some(z=>b.currentPrice>Math.min(z.a,z.b));
      if(!stillInAny){
  ...
  // M1 must be downtrend to watch for reversals
  if(b.trendStatus['M1']!=='down') return;

  // Check M1, M5, M15 — any one triggers pause
  let uptrendDetected=false;
  for(const tf of ['M1','M5','M15']){
    const tfData=b.candles[tf];
    if(!tfData||tfData.length<20) continue;
    const recent=tfData.slice(-30);
    const sLows=[], sHighs=[];
    ...
    if(sLows.length>=2&&sHighs.length>=1){
      ...
      // Pattern 1: Higher Low + Break Above Swing High
      if(lastLow.price>prevLow.price&&lastLow.idx>prevLow.idx){
        if(b.currentPrice>lastHigh.price&&lastHigh.idx>prevLow.idx){
          uptrendDetected=true; break;
        }
      }
      // Pattern 2: Lower Low + Break Above Its Own High
      if(lastLow.price<prevLow.price&&lastLow.idx>prevLow.idx){
  ...
  if(uptrendDetected&&!b.htfZonePaused){
    ...
    setTicker(b,`⚠ Uptrend in HTF zone ${lo.toFixed(2)}–${hi.toFixed(2)} — paused`);
    telegram(b,`⚠ <b>Bot paused</b>\nUptrend structure in HTF zone\nResumes when price breaks below zone or 2+ TFs uptrend`);
    broadcastBotState(b);
  }

  // Resume when at least 2 of M1/M5/M15 turn uptrend
  const upCount=Object.values(b.trendStatus).filter(t=>t==='up').length;
  if(b.htfZonePaused&&upCount>=2){
    log(b,'✅ 2+ TFs uptrend — HTF zone resolved');
    b.htfZonePaused=false;
    setStatus(b,'running','RUNNING');
    setTicker(b,'✅ 2+ TFs uptrend — waiting for next downtrend...');
    broadcastBotState(b);
  }
}
...
function placeTrade(b) {
  if(!b.derivWs||b.derivWs.readyState!==WebSocket.OPEN){
    log(b,'❌ placeTrade: WebSocket not open');
    b.inTrade=false; return;
  }
  b.inTrade=true;
  ...
  if(won){
    ...
    if(b.consecutiveLosses>=b.cfg.maxConsecLosses){
      ...
      if(b.cfg.cooldownEnabled!==false){
        ...
        setTicker(b,'❌ LOSS — no cooldown, resuming...');
        setTimeout(()=>{if(b.botActive)findLevels(b);},1000);
      }
    }
  }
...
function connectDerivLive(b, wssUrl) {
  ...
  b.derivWs.on('open', () => {
    ...
    ['M1','M5','M15','M30','H1','H4','D1'].forEach(tf=>fetchCandles(b,tf));
    startScanner(b);
    broadcastBotState(b);
  });
...
function handleDerivMessage(b, d) {
  ...
  if(d.msg_type==='proposal'){
    if(b.inTrade) handleProposal(b,d);
    return;
  }
  ...
  if(d.msg_type==='buy'){
    if(d.error){
      ...
      log(b,`❌ Buy error detail: ${JSON.stringify(d.error)}`);
      b.inTrade=false; broadcastBotState(b); return;
    }
    b.currentContractId=d.buy.contract_id;
    log(b,`📝 Contract opened: ${b.currentContractId} | entry: ${d.buy.start_time}`);
    const isMulti=b.cfg.command==='CALL_MULT'||b.cfg.command==='PUT_MULT';
    const duration=b.cfg.durationMins*60;
    if(isMulti){
      setTimeout(()=>{
